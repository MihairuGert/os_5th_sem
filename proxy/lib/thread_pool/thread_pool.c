...
static int is_thread_running(thread_info_t* thread)
{
    int result;
...
static void* deamon_routine(void* arg)
{
...
static int create_deamon(thread_pool_t* t_pool)
{
    int err;
...
static int create_thread(thread_info_t* thread,
    ...
    void* arg)
{
    ...
    t_pool->threads = realloc(t_pool->threads,
        sizeof(thread_info_t) * REALLOC_MAGIC * t_pool->thread_count);
    if (!t_pool->threads) {
        pthread_mutex_unlock(&t_pool->mutex);
    ...
    err = create_thread(&t_pool->threads[t_pool->thread_count / REALLOC_MAGIC],
