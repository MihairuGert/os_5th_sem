...
int 
parse_http_req(http_req_t* req, char *bytes, int size)
{
    bytes[size] ='\0';  
...
int 
parse_http_resp(http_resp_t* resp, char *bytes, int size)
{
    bytes[size] ='\0';  
    resp->num_headers = sizeof(resp->headers) / sizeof(resp->headers[0]);

    int bytes_used = phr_parse_response(
        bytes, size, 
        &resp->minor_version, 
        &resp->status,
        &resp->msg, &resp->msg_len, 
