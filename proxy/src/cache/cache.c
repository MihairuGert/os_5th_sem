...
create_entry(const char *key, size_t initial_size, time_t ttl)
{
...
static void
free_entry(cache_entry_t * entry)
{
	if (entry)
	{
		free(entry->key);
...
cache_status_t
cache_lookup(cache_t * cache, const char *key, char **data, size_t *data_size)
{
	bool		is_loading;
...
cache_entry_t *
cache_start_loading(cache_t * cache, const char *key, time_t ttl)
{
	unsigned long bucket_idx = cache_hash(key, cache->num_buckets);
	cache_bucket_t *bucket = &cache->buckets[bucket_idx];
...
int
cache_finish_loading(cache_entry_t * entry)
{
	if (!entry || !atomic_load(&entry->is_loading))
	{
		return -1;
...
int
cache_cancel_loading(cache_t * cache, const char *key)
{
	unsigned long bucket_idx = cache_hash(key, cache->num_buckets);
	cache_bucket_t *bucket = &cache->buckets[bucket_idx];
...
void
cache_clean_expired(cache_t * cache)
{
	time_t		now = time(NULL);
...
unsigned long
cache_hash(const char *key, size_t num_buckets)
{
...
static void
notify_waiting_clients(cache_entry_t * entry)
{
	if (!entry)
		return;
	...
	while (waiter)
	{
		if (waiter->async_handle
			&& !uv_is_closing((uv_handle_t *) waiter->async_handle))
		{
			printf(
				   "[BGW %d] Send notify to %p\n",
				   gettid(),
				   waiter->async_handle);
...
static void
free_waiters_list(cache_waiter_t * waiters)
{
	while (waiters)
	{
					  ...
					  uv_async_t * async_handle)
{
	...
	while (entry)
	{
		if (strcmp(entry->key, key) == 0)
		{
			...
			printf(
				   "[Backend %d] Register waiter %p to %s\n",
				   gettid(),
				   async_handle,
				   key);
			...
			pthread_mutex_unlock(&bucket->mutex);
			printf(
				   "[Backend %d] Successfully registered waiter %p to %s\n",
				   gettid(),
				   async_handle,
				   key);
			return 0;
	...
	cache_waiter_table_entry_t *new_entry =
		malloc(sizeof(cache_waiter_table_entry_t));
