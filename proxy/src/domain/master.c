...
static backend_t * get_backend(master_t * master)
{
	...
	printf(
		   "[Master %d] Current backends count = %ld\n",
		   gettid(),
		   master->cur_backend_count);

	for (size_t i = 0; i < master->cur_backend_count; i++)
	{
		bool		can_accept = atomic_load(&master->backends[i].client_count)
			< MAX_BACKEND_CONNECTIONS_NUM;

		printf(
			   "[Master %d] Current backend (%ld) clients count = %d\n",
			   gettid(),
			   i,
			   master->backends[i].client_count);
