...
static void
read_client(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf);
static write_req_t* alloc_write_req_t(
    backend_t* backend,
    uv_stream_t* client,
    char* path,
    char* data,
...
void* run_backend(void* backend)
{
    ...
    uv_read_start(
	(uv_stream_t*)&cast_backend->fd_pipe,
	alloc_pipe_buf,
	read_pipe);
...
static void read_client(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf)
{
    int err;
	...
	for (int i = 0; i < http_req->num_headers; i++)
	{
	    if (strncmp(
		    "Referer",
		    http_req->headers[i].name,
		    http_req->headers[i].name_len)
		== 0)
	    {
		referer = strndup(
		    http_req->headers[i].value,
		    http_req->headers[i].value_len);
	    }
	}
	...
	cache_status_t status =
	    cache_lookup(backend->cache, key, &data, &data_size);
	switch (status)
	{
	    case CACHE_FOUND:
	    {
		printf(
		    "[Backend %d] Data found in cache, starting sending for: %s\n",
		    gettid(),
		    key);
		write_req_t* req =
		    alloc_write_req_t(backend, client, key, data, data_size);
	    ...
	    {
		printf(
		    "[Backend %d] Data not found in cache, starting background load for: %s\n",
		    gettid(),
		    key);
	    ...
	    {
		printf(
		    "[Backend %d] Data is loading, starting streaming for: %s\n",
		    gettid(),
		    key);
		write_req_t* req =
		    alloc_write_req_t(backend, client, key, data, data_size);
...
static void send_next_chunk(write_req_t* wr)
{
    ...
    cache_status_t status = cache_lookup_partial(
	wr->backend->cache,
	wr->path,
	&data,
	&data_size,
    ...
    printf(
	"[Backend %d] cache_lookup_partial-> status=%d, data_size=%zu is_loading=%d\n",
	gettid(),
	status,
	data_size,
	still_loading);
	...
	case CACHE_FOUND:
	    ...
	    {
		printf(
		    "[Backend %d] No data available yet for %s, registering for notifications\n",
		    gettid(),
		    wr->path);
		if (!wr->did_register)
		{
		    cache_register_waiter(
			wr->backend->cache,
			wr->path,
			&wr->data_ready_async);
	...
	case CACHE_NOT_FOUND:
	    printf(
		"Data not found for %s, registering for notifications...\n",
		wr->path);
	    if (!wr->did_register)
	    {
		cache_register_waiter(
	...
	case CACHE_NEW_LOADED:
	    printf(
		"[Backend %d] Data still loading for %s\n",
		gettid(),
		wr->path);
	    if (data_size > 0)
	    {
		...
		printf(
		    "[Backend %d] Send data to client path=%s\n",
		    gettid(),
		    wr->path);
		wr->buf = uv_buf_init(data, data_size);
		wr->progress.last_offset += data_size;
...
static void new_client(backend_t* backend, uv_tcp_t* client)
{
    ...
    printf(
	"[Backend %d] New client connected, total clients: %d\n",
	gettid(),
...
static void on_close(uv_handle_t* handle)
{
    ...
    atomic_fetch_add(&backend->client_count, -1);
    printf(
	"[Backend %d] Client disconnected, remaining clients: %d\n",
	gettid(),
	atomic_load(&backend->client_count));
...
static void echo_write_buffer_load(uv_write_t* req, int status)
{
	...
	{
	    printf(
		"[Backend %d] Client disconnected (broken pipe) for %s\n",
		gettid(),
		wr->path);
	}
	free_write_req(wr);
    ...
    if (wr->progress.is_loading
	|| wr->progress.last_offset < wr->progress.total_size)
    {
	send_next_chunk(wr);
    }
    else
    {
	printf(
	    "All data sent for %s, total: %zu bytes\n",
	    wr->path,
	    wr->progress.last_offset);
	free_write_req(wr);
    }
...
void free_write_req(write_req_t* wr)
{
    ...
    printf(
	"[Backend %d] Start closing connection for %s...\n",
	gettid(),
	wr->path ? wr->path : "unknown");
    if (wr->path && wr->was_async_init)
    {
	cache_unregister_waiter(
	    wr->backend->cache,
	    wr->path,
	    &wr->data_ready_async);
    }
