    ...
    void* userdata)
{
    ...
    if (ctx->entry && atomic_load(&ctx->entry->is_loading)) {
        ...
        printf("[BGW %d] Appended %zu bytes to cache, total: %zu\n", gettid(),
...
static void load_http_resp(uv_work_t* req)
{
    ...
    ctx.entry = cache_start_loading(task->backend->cache, task->key, DEFAULT_TTL);
    if (!ctx.entry) {
    ...
    if (res != CURLE_OK) {
        fprintf(stderr, "[BGW %d] CURL failed for %s: %s\n", gettid(), path,
        ...
        printf("[BGW %d] Request completed for %s: HTTP %ld, Total bytes: %zu\n",
            gettid(), path, http_code, ctx.total_size);

        if (http_code == 200) {
            printf("[BGW %d] Successfully loaded %zu bytes into cache\n", gettid(),
                ctx.total_size);
    ...
    printf("[BGW %d] HTTP load completed for %s with result: %d\n", gettid(),
        path, task->result);
}

static void after_load_http_resp_cb(uv_work_t* req, int status)
{
    task_t* task = (task_t*)req->data;

    printf("[BGW] Background work completed, status: %d, result: %d\n", status,
        task->result);
    free_task(task);
}
...
int launch_bgw(task_t* task)
{
    ...
    printf("[BGW] Launching background worker for path %*.s\n with a key=%s",
        (int)task->req->path_len, task->req->path, task->key);

    err = uv_queue_work(&task->backend->loop, &task->work_req, load_http_resp,
