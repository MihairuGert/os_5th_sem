...
Node* create_node(const char* value) {
    ...
    int init_result = mutex_init(&new_node->sync);
    if (init_result != 0) {
        fprintf(stderr, "Error: Failed to initialize node mutex: %s\n", strerror(init_result));
        free(new_node);
        return NULL;
    }
...
void* count_ascending(void* arg) {
    ...
    while (!stop) {
        ...
        while (current != NULL && current->next != NULL && !stop) {
            ...
            int unlock_result2 = mutex_unlock(&next->sync);
            if (unlock_result2 != 0) {
...
int safe_swap_nodes(Storage* storage, int thread_id) {
    ...
    while (!stop) {
        ...
        mutex_lock(&storage->first->sync);
        prev = storage->first;
        
        int ind = 1;
        while (ind < randElem && prev != NULL && prev->next != NULL) {
            mutex_lock(&prev->next->sync);
            Node* temp = prev;
            prev = prev->next;
            mutex_unlock(&temp->sync);
            ind++;
        }
        
        if (prev == NULL || prev->next == NULL || prev->next->next == NULL) {
            if (prev != NULL) mutex_unlock(&prev->sync);
            continue;
        }
        
        mutex_lock(&prev->next->sync);
        current = prev->next;
        mutex_lock(&current->next->sync);
        next = current->next;
        ...
        mutex_unlock(&prev->sync);
        mutex_unlock(&current->sync);
        mutex_unlock(&next->sync);
